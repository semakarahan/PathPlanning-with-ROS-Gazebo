...
def greedy_bfs(start_index, goal_index, width, height, costmap, resolution, origin, grid_viz):
  ...
  xy_start = indexToWorld(start_index, width, resolution, map_origin = [0,0])
  xy_goal = indexToWorld(goal_index, width, resolution, map_origin = [0,0])
  h_cost = euclidean_distance(xy_start, xy_goal)
  ...
  while open_list:
    ...
    for neighbor_index, step_cost in neighbors:
      ...
      xy_neighbor = indexToWorld(neighbor_index, width, resolution, map_origin=[0, 0])
      xy_goal = indexToWorld(goal_index, width, resolution, map_origin=[0, 0])
      h_cost = euclidean_distance(xy_neighbor, xy_goal)
