...
def a_star(start_index, goal_index, width, height, costmap, resolution, origin, grid_viz):
  ...
  # dict for mapping f costs (heuristic costs) to nodes
  g_costs = dict()
  ...
  f_cost = h_cost + 0
  # set the start's node f_cost
  f_costs[start_index] = f_cost

  # add start node to open list
  open_list.append([start_index, f_cost])
